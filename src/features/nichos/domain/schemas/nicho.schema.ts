...
const NichoBaseSchema = z.object({
  ...
  observaciones: z
    .nullable(z.string().max(500, "Máximo 500 caracteres"))
    .optional(),
...
export const UpdateNichoSchema = NichoBaseSchema.partial()
  .extend({
  ...
  .refine(
    (data) => {
      if (data.tipo === "Mausoleo") {
        return (
          data.numHuecos === undefined ||
          (data.numHuecos >= 1 && data.numHuecos <= 9)
        );
      }
