...
export function usePersonForm({ person, onSuccess }: UsePersonFormOptions = {}) {
  ...
  const onSubmit = (data: CreatePersonDTO) => {
    ...
    if (person && person.id_persona && person.fallecido === true && data.fallecido === false) {
      // Force fallecido to remain true
      data.fallecido = true;
    }
