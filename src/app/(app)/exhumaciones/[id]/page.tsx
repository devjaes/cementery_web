...
import {
  useUploadComprobanteMutation,
  useDeleteExhumacionMutation,
} from "@/features/exhumaciones/presentation/hooks/use-exhumacion-mutations";
import {
  useCreatePayment,
  useDownloadReceipt,
  useUploadReceipt,
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-gray-500">
                  Tamaño: {(exhumacion.archivos.data.length / 1024).toFixed(1)}{" "}
                  KB
                  {exhumacion.archivos.data[0] === 37 &&
                  exhumacion.archivos.data[1] === 80
                    ? " • PDF"
                    : " • Archivo binario"}
                </span>
              </div>
            </div>
            <Button variant="outline" size="sm" onClick={handleDownloadArchivo}>
...
export default function ExhumacionDetailPage(): React.JSX.Element {
  ...
  const [authorizationPdfUrl, setAuthorizationPdfUrl] = useState<string | null>(
    null
  );
  ...
  const {
    ...
    isLoading,
    error,
  } = useFindExhumacionByIdQuery(exhumacionId);
  ...
  if (exhumacion) {
    ...
    console.log(" Campo archivos:", {
      ...
      propiedades: exhumacion.archivos
        ? Object.keys(exhumacion.archivos)
        : "No disponible",
    });
    ...
    console.log(" Campos faltantes en BD (hardcodeados):", {
      ...
      "Para PDF autorización": [
        "parentescoDuenio",
        "fechaAdquisicionNicho",
        "administradorCementerio",
        "tipoPropiedad",
        "directorServicios",
    ...
    try {
      ...
      if (payments?.length) {
        ...
        if (pendingPayment) {
          ...
          console.log(
            " No se encontró pago pendiente en el módulo de payments, solo se actualizó la tabla de exhumaciones"
          );
        }
      } else {
        console.log(
          " No hay pagos en el módulo de payments, solo se actualizó la tabla de exhumaciones"
        );
      }
  ...
  const handleDelete = async () => {
    ...
    if (
      confirm(
        "¿Estás seguro de que deseas eliminar esta exhumación? Esta acción no se puede deshacer."
      )
    ) {
      try {
  ...
  const handleDownloadArchivo = () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `exhumacion-${
        exhumacion.codigo || exhumacion.idExhumacion
      }-documento.${
        exhumacion.archivos.data[0] === 37 && exhumacion.archivos.data[1] === 80
          ? "pdf"
          : "bin"
      }`;
  ...
  const handleDownloadComprobante = () => {
    if (
      ...
      exhumacion.comprobantePago.type !== "Buffer" ||
      !exhumacion.comprobantePago.data
    ) {
    ...
    try {
      ...
      const blob = new Blob([byteArray], {
        type:
          exhumacion.comprobantePago.data[0] === 37 &&
          exhumacion.comprobantePago.data[1] === 80
            ? "application/pdf"
            : "application/octet-stream",
      });
  ...
  const handleDownloadAutorizacion = async () => {
    ...
    try {
      ...
      const getCleanUbicacion = () => {
        const rawCementerio =
          exhumacion.inhumacion?.idNicho?.idCementerio?.nombre ??
      ...
      const drawTextInCell = (
        ...
        lines.forEach((line: string, index: number) => {
          ...
          if (lineY < y + cellHeight - 1) {
            // Verificar que no se salga de la celda
      ...
      pdf.text("GADM SANTIAGO DE PÍLLARO", pageWidth / 2, yPos + 8, {
        align: "center",
      });

      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(9);
      pdf.text("DIRECCIÓN DE SERVICIOS PÚBLICOS", pageWidth / 2, yPos + 14, {
        align: "center",
      });
      ...
      const fechaTexto = `FECHA: ${formatDateSafely(
        new Date(),
        "dd 'DE' MMMM 'DE' yyyy"
      ).toUpperCase()}`;
      ...
      pdf.text(
        encabezadoA,
        leftMargin + (tableWidth * 0.5) / 2,
        yPos + encabezadosHeight / 2 + 1,
        { align: "center" }
      );

      drawTableBorder(
        leftMargin + tableWidth * 0.5,
        yPos,
        tableWidth * 0.5,
        encabezadosHeight,
        false
      ...
      pdf.text(
        encabezadoB,
        leftMargin + tableWidth * 0.5 + (tableWidth * 0.5) / 2,
        yPos + encabezadosHeight / 2 + 1,
        { align: "center" }
      );
      ...
      const institutionalRows = [
        [
          "Cementerio:",
          ubicacionLimpia || "CEMENTERIO MUNICIPAL CENTRAL",
          "Escrito",
          true,
        ],
        [
          "Funcionario o cargo:",
          "WILSON HINOJOSA",
          "Verbal (solo en caso de emergencia)",
          false,
        ], // TODO: Debe venir de BD - tabla funcionarios/empleados
      ];
      ...
      pdf.text(
        "Cumple",
        leftMargin + tableWidth * 0.77 + (tableWidth * 0.08) / 2,
        yPos + encabezadosHeight2 / 2 + 1,
        { align: "center" }
      );

      drawTableBorder(
        leftMargin + tableWidth * 0.85,
        yPos,
        tableWidth * 0.08,
        encabezadosHeight2,
        false
      ...
      pdf.text(
        "No cumple",
        leftMargin + tableWidth * 0.85 + (tableWidth * 0.08) / 2,
        yPos + encabezadosHeight2 / 2 + 1,
        { align: "center" }
      );

      drawTableBorder(
        leftMargin + tableWidth * 0.93,
        yPos,
        tableWidth * 0.07,
        encabezadosHeight2,
        false
      ...
      pdf.text(
        "Observación",
        leftMargin + tableWidth * 0.93 + (tableWidth * 0.07) / 2,
        yPos + encabezadosHeight2 / 2 + 1,
        { align: "center" }
      );
      ...
      const solicitanteRequisitos = [
        [
          "Nombre/Apellido",
          exhumacion.duenioNicho || "CAMPAÑA PÁEZ GLORIA TERESA",
          "Copia del certificado de defunción REC",
          true,
          false,
          "",
        ],
        [
          "Parentesco (únicamente de primer grado)",
          "SOBRINA",
          "Certificado de Inhumación",
          true,
          false,
          "",
        ], // TODO: Debe venir de BD - campo parentesco
        [
          "Nº Cédula de Identidad",
          exhumacion.inhumacion?.idFallecido?.cedula || "1800846784",
          "Copia de C.I. del solicitante",
          true,
          false,
          "",
        ], // TODO: Debe ser cédula del dueño/solicitante
        [
          "",
          "",
          "Copia del T. de propiedad del nicho/lote/sitio",
          true,
          false,
          "USUARIO CUENTA CON DOCUMENTO OTORGADO POR EL SERVICIOS PÚBLICOS. OFICIO DE INDUCCIÓN DE LA DIRECCIÓN MUNICIPAL",
        ],
        [
          "Dirección",
          "PÍLLARO CENTRO",
          ...
          false,
          "",
        ], // TODO: Debe venir de BD - direccion del dueño
        [
          "Num. Celular",
          "88333634",
          "Haber cumplido 4 años de inhumación",
          true,
          false,
          "",
        ], // TODO: Debe venir de BD - telefono del dueño
        [
          "Correo Electrónico",
          "",
          "Orden de un juez (en caso de efectos legales)",
          false,
          false,
          "N/A",
        ], // TODO: Debe venir de BD - email del dueño
        [
          "",
          "",
          "Pago por exhumación",
      ...
      solicitanteRequisitos.forEach((row) => {
        ...
        const col1Height = calculateTextHeight(
          row[0] as string,
          tableWidth * 0.22 - 4,
          7
        );
        const col2Height = calculateTextHeight(
          row[1] as string,
          tableWidth * 0.28 - 4,
          7
        );
        const col3Height = calculateTextHeight(
          row[2] as string,
          tableWidth * 0.27 - 4,
          7
        );
        const col6Height = calculateTextHeight(
          row[5] as string,
          tableWidth * 0.07 - 4,
          6
        );
        ...
        const currentCellHeight = Math.max(
          col1Height,
          col2Height,
          col3Height,
          col6Height,
        ...
        if (row[3]) {
          ...
          pdf.text(
            "X",
            leftMargin + tableWidth * 0.81,
            yPos + currentCellHeight / 2 + 1
          );
        }
      ...
      const encabezadoEHeight = calculateTextHeight(
        encabezadoE,
        tableWidth - 4,
        8
      );
      ...
      pdf.text(
        encabezadoE,
        leftMargin + tableWidth / 2,
        yPos + encabezadoEHeight / 2 + 1,
        { align: "center" }
      );
      ...
      const nichoRows = [
        [
          ...
          "Número de nicho",
          "",
        ], // TODO: Número debe venir del nicho relacionado
        [
          "Fecha de adquisición",
          "17 DE NOVIEMBRE DE 1974",
          "Lugar del nicho",
          ubicacionLimpia || "CEMENTERIO MUNICIPAL CENTRAL",
        ], // TODO: Debe venir de BD - fecha adquisicion del nicho
        ...
        [
          ...
          "Arrendado/a",
          "",
          "Firma de aceptación de exhumación",
          "",
        ], // TODO: Debe venir de BD - tipo de propiedad del nicho
      ];

      nichoRows.forEach((row, index) => {
        if (index === 3) {
          // Última fila con span adicional
          const rowHeight = Math.max(
      ...
      const encabezadoFHeight = calculateTextHeight(
        encabezadoF,
        tableWidth - 4,
        8
      );
      ...
      const fallecidoRows = [
        [
          "Nombre/Apellido",
          `${exhumacion.inhumacion?.idFallecido?.nombres || "LUZ EDELMIRA"} ${
            exhumacion.inhumacion?.idFallecido?.apellidos || ""
          }`,
          "FECHA DE EXHUMACIÓN",
          formatDateSafely(
            exhumacion.fechaExhumacion,
            "dd 'DE' MMMM 'DE' yyyy"
          ).toUpperCase(),
        ],
        [
          ...
          formatDateSafely(
            exhumacion.inhumacion?.fechaInhumacion,
            "dd 'DE' MMMM 'DE' yyyy"
          ).toUpperCase() || "08 DE NOVIEMBRE DE 2008",
          "Hora de exhumación",
          exhumacion.horaExhumacion || "15H00",
        ],
        [
          "Fecha de Inhumación",
          formatDateSafely(
            exhumacion.inhumacion?.fechaInhumacion,
            "dd 'DE' MMMM 'DE' yyyy"
          ).toUpperCase() || "10 DE NOVIEMBRE DE 2008",
          "Lugar de nueva sepultura (nicho, lote,lugar)",
          "CEMENTERIO MUNICIPAL DE CALULÉ",
        ],
        [
          "Lugar de sepultura (nicho, lote,lugar)",
          ubicacionLimpia || "CEMENTERIO MUNICIPAL CALULÉ",
          "",
          "",
        ],
        [
          "N° Cédula de Identidad",
          exhumacion.inhumacion?.idFallecido?.cedula || "",
          "Aprobación de los familiares",
          "LA SOLICITANTE ES SOBRINA DE LA PERSONA FALLECIDA POR LO QUE ADJUNTA AUTORIZACIÓN DE LOS HIJOS DE LA PERSONA FALLECIDA.",
        ],
      ];

      fallecidoRows.forEach((row, index) => {
        if (index === 4) {
          // Última fila con span especial
          const rowHeight = Math.max(
            calculateTextHeight(row[0], tableWidth * 0.25 - 4, 7),
            calculateTextHeight(row[1], tableWidth * 0.25 - 4, 7),
            calculateTextHeight(row[2], tableWidth * 0.15 - 4, 7),
      ...
      const autorizationText =
        "A petición del solicitante y habiendo cumplido con los requisitos de Ley, Se AUTORIZA la EXHUMACIÓN EN EL CEMENTERIO MUNICIPAL CENTRAL";
      const authHeight = calculateTextHeight(
        autorizationText,
        tableWidth - 4,
        6
      );
      ...
      // === NOTA EN CELDA ===
      const notaText =
        "Nota: Se debe indicar que se ha socializado con los deudos que para realizar exhumaciones está prohibido para todos los que fallecieron a partir del 2020.";
      ...
      pdf.setFont("helvetica", "normal");
      const notaTextoSolo =
        "Se debe indicar que se ha socializado con los deudos que para realizar exhumaciones está prohibido para todos los que fallecieron a partir del 2020.";
      drawTextInCell(
        notaTextoSolo,
        leftMargin + 12,
        yPos,
      ...
      const ordenanzaHeight = calculateTextHeight(
        ordenanzaText,
        tableWidth - 4,
        6
      );
      ...
      pdf.line(
        leftMargin + 10,
        yPos + 29,
        leftMargin + tableWidth * 0.5 - 10,
        yPos + 29
      );
      pdf.setFont("helvetica", "normal");
      const solicitanteNombre =
        exhumacion.duenioNicho || "CAMPAÑA PÁEZ GLORIA TERESA";
      pdf.text(solicitanteNombre, leftMargin + tableWidth * 0.25, yPos + 32, {
        align: "center",
      });
      ...
      pdf.text(
        ...
        yPos + 32,
        { align: "center" }
      ); // TODO: Debe venir de BD - director de servicios públicos

      // === BORDE FINAL DEL DOCUMENTO ===
      // Dibujar borde completo en todas las páginas
      const totalPages =
        (pdf.internal as { pages: unknown[] }).pages.length - 1; // -1 porque el array incluye un elemento vacío al inicio
      ...
      console.log(" Autorización PDF generada en memoria:", {
        size: pdfBlob.size,
        type: pdfBlob.type,
        url: pdfUrl,
      });
      ...
      alert(
        "Error al generar el PDF de autorización. Por favor, inténtalo nuevamente."
      );
    } finally {
  ...
  const handleGeneratePaymentOrder = async () => {
    ...
    if (paymentAmount <= 0) {
      alert(
        "Error: Debes establecer un monto mayor a $0.00 antes de generar la orden de pago."
      );
      return;
    }
    ...
    try {
      ...
      if (!paymentToUse) {
        ...
        const fallecidoCompleto = `${
          exhumacion.inhumacion?.idFallecido?.nombres || ""
        } ${exhumacion.inhumacion?.idFallecido?.apellidos || ""}`.trim();
        ...
        const buyerDocument =
          exhumacion.inhumacion?.idFallecido?.cedula || "1800846784"; // TEMPORAL: usar cédula del fallecido como fallback
        const buyerDirection =
          exhumacion.inhumacion?.idFallecido?.direccion || "PÍLLARO CENTRO"; // TEMPORAL: usar dirección del fallecido como fallback
        ...
        console.log(" Datos del pago a enviar:", {
          ...
          " Campos usando datos temporales": {
            ...
            buyerDirection_fuente:
              "fallecido.direccion (debería ser dueño.direccion)",
            campos_faltantes_en_BD: [
              "cedulaDuenio",
              "direccionDuenio",
              "telefonoDuenio",
            ],
      ...
      if (paymentToUse) {
        ...
        (document.createElement as any) = function (
          tagName: string,
          ...
          if (tagName.toLowerCase() === "a" && interceptedPdfUrl) {
            console.log(
              " Enlace <a> creado durante interceptación - modificando comportamiento"
            );
        ...
        URL.createObjectURL = function (object: Blob | MediaSource): string {
          ...
          if (
            ...
            (object.type === "application/pdf" || object.size > 5000)
          ) {
            // Probablemente un PDF
            console.log("  PDF Blob detectado y interceptado:", {
              type: object.type,
              size: object.size,
              url,
            });
            ...
            console.log(
              "  Estados establecidos con URL independiente - Modal debería aparecer ahora"
            );
          }
        ...
        (document.body.appendChild as any) = function (node: Node): Node {
          if (node instanceof HTMLAnchorElement) {
            ...
            if (
              ...
              console.log(
                " BLOQUEANDO descarga automática - Solo mostrar en modal"
              );
              downloadIntercepted = true;
        ...
        try {
          ...
          if (interceptedPdfUrl && downloadIntercepted) {
            ...
            console.log(
              "  Los estados ya fueron establecidos en la interceptación"
            );
          } else {
            console.log(
              "  No se interceptó el PDF, verificar implementación del hook"
            );
          }
        } finally {
  ...
  const handleDownloadPaymentPdf = () => {
    if (paymentPdfUrl) {
      ...
      link.download = `orden-pago-exhumacion-${
        exhumacion?.codigo || exhumacion?.idExhumacion
      }.pdf`;
      document.body.appendChild(link);
      link.click();
  ...
  const handleClosePaymentModal = (open: boolean) => {
    console.log(
      " handleClosePaymentModal llamado con open:",
      open,
      "paymentPdfUrl actual:",
      paymentPdfUrl
    );

    if (!open) {
      // Solo cerrar cuando open es false
  ...
  const handleCloseAuthorizationModal = (open: boolean) => {
    console.log(
      " handleCloseAuthorizationModal llamado con open:",
      open,
      "authorizationPdfUrl actual:",
      authorizationPdfUrl
    );
  ...
  const handleDownloadAuthorizationPdf = () => {
    if (authorizationPdfUrl) {
      ...
      link.download = `autorizacion-exhumacion-${
        exhumacion?.codigo || exhumacion?.idExhumacion
      }.pdf`;
      document.body.appendChild(link);
      link.click();
    ...
    return (
      <ContainerApp title="Detalles de Exhumación">
        <Alert>
          ...
          <AlertDescription>
            Error al cargar los detalles de la exhumación. Por favor intenta
            nuevamente.
          </AlertDescription>
        </Alert>
      </ContainerApp>
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge
              variant={
                exhumacion.estadoPago === "finalizado" ? "default" : "secondary"
              }
              className={
                exhumacion.estadoPago === "finalizado"
                  ? "bg-green-600"
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-sm">
                      {(() => {
                        ...
                        const rawCementerio =
                          exhumacion.inhumacion?.idNicho?.idCementerio
                            ?.nombre ??
                          exhumacion.ubicacion ??
                          "CEMENTERIO MUNICIPAL";
                        ...
                        const extractCemeteryName = (s: string) => {
                          ...
                          let cleaned = s.replace(
                            /\b(Fila|Columna)\b\s*:\s*null/gi,
                            ""
                          );
                          cleaned = cleaned.replace(
                            /\b(Fila|Columna)\b\s*null\b/gi,
                            ""
                          );
                          // Remover múltiples guiones y espacios sobrantes
                          cleaned = cleaned.replace(/\s*-\s*-+/g, "-");
                        ...
                        const cementerio = extractCemeteryName(
                          String(rawCementerio)
                        );

                        const rawFila = exhumacion.inhumacion?.idNicho?.fila;
                        const rawColumna =
                          exhumacion.inhumacion?.idNicho?.columna;
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="text-sm text-gray-600">
                    ...
                    {exhumacion.inhumacion.codigoInhumacion || "No disponible"}
                  </div>
                </div>
                <div className="space-y-2">
                  <div className="text-sm text-gray-600">
                    <span className="font-medium">Fecha de Inhumación:</span>{" "}
                    {formatDateSafely(exhumacion.inhumacion.fechaInhumacion)}
                  </div>
                  <div className="text-sm text-gray-600">
                    <span className="font-medium">Solicitante:</span>{" "}
                    {exhumacion.inhumacion.solicitante || "No disponible"}
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        {/* @ts-expect-error - ArchivosSection is a valid component */}
        <ArchivosSection
          exhumacion={exhumacion}
          handleDownloadArchivo={handleDownloadArchivo}
        />

        {/* @ts-expect-error - ConfigMontoSection is a valid component */}
        <ConfigMontoSection
          exhumacion={exhumacion}
          payments={payments || []}
            ...
            payments?.some((p) => p.status === "paid") ? (
              <div className="space-y-4">
                ...
                >
                  ...
                  {generatingAuthorization
                    ? "Generando Autorización..."
                    : "Visualizar Autorización de Exhumación"}
                </Button>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                {paymentAmount === 0 && (
                  <div className="bg-orange-50 p-4 rounded border border-orange-200 text-center">
                    <p className="text-orange-700 text-sm">
                      ⚠️ Establece primero el monto de la exhumación en la
                      sección anterior
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-3">
                  ...
                  {paymentAmount <= 0 && (
                    <p className="text-xs text-red-600 bg-red-50 p-2 rounded">
                      ⚠️ Debes establecer un monto mayor a $0.00 antes de
        ...
        {console.log(
          "🎭 Renderizando modal - showPaymentModal:",
          showPaymentModal,
          "paymentPdfUrl:",
          paymentPdfUrl
        )}
            ...
            <div className="flex-1 min-h-0">
              {paymentPdfUrl ? (
                <iframe
                  ...
                  onError={(e) =>
                    console.error(" IFRAME: Error al cargar PDF", e)
                  }
        ...
        {console.log(
          "📋 Renderizando modal autorización - showAuthorizationModal:",
          showAuthorizationModal,
          "authorizationPdfUrl:",
          authorizationPdfUrl
        )}
        <Dialog
          ...
          onOpenChange={handleCloseAuthorizationModal}
        >
          <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
            ...
            <div className="flex-1 min-h-0">
              {authorizationPdfUrl ? (
                <iframe
                  ...
                  onLoad={() =>
                    console.log(
                      "📋 IFRAME: Autorización PDF cargada exitosamente"
                    )
                  }
                  onError={(e) =>
                    console.error(
                      "❌ IFRAME: Error al cargar Autorización PDF",
                      e
                    )
                  }
                  onAbort={() =>
                    console.log("⏹️ IFRAME: Carga de autorización abortada")
                  }
                />
              ) : (
                <div className="flex items-center justify-center h-64">
